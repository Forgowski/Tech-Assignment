...
def assign_the_user(request, id):
    ...
    if Device.is_any_assigned_and_active(user_id):
        return Response(
            {"user_id": ["A user can have only one assigned and active device at a time."]},
...
def set_location(request, id):
    ...
    serializer = DeviceSetLocationSerializer(instance=device, data=request.data, partial=True)
    if serializer.is_valid():
        serializer.save()
